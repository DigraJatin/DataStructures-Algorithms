...
*/
vector<int> nextGreaterRight_LtoR(const vector<int>& nums) {
    int n = nums.size();
...
*/
vector<int> nextGreaterRight_RtoL(const vector<int>& nums) {
    int n = nums.size();
    vector<int> result(n, -1);
    stack<int> st; // stores indices
...
int main() {
    ...
    for (auto& arr : tests) {
        ...
        auto r1 = nextGreaterRight_LtoR(arr);
        auto r2 = nextGreaterRight_RtoL(arr);
